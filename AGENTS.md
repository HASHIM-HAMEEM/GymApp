...
- Admin creates a member in the app → `create-member-invitation` Edge Function → `auth.admin.generateLink` + Resend → member gets a one-time email link and the Android APK download link.
...
- `functions/` — Deno Edge Functions (`create-member-invitation`, `resend-member-invitation`, `update-member-email`) with `_shared/` helpers. Pinned `npm:@supabase/supabase-js@2.112.4`. Checked with the shim in `runtime.d.ts` (see Verification below).
...
- **Member identity rules (write-once + fresh re-enrolment).** Email and Aadhaar are unique among non-removed members and immutable once activated. Admins may correct email only before onboarding via `update_member_email`. A removed person may join from scratch with the same email/Aadhaar: a new `APX-XXXXXX` member, auth account and invitation are created; the old auth user is retired by `prepare_removed_member_reenrolment`, the old row points to `reenrolled_as` and remains read-only history with all old memberships/payments/visits. It cannot be restored after re-enrolment. Aadhaar still cannot be edited on one record (`members_national_id_write_once`). New registration requires mobile + gender; legacy rows use `unspecified`. Duplicate active Aadhaar/email map to `AADHAAR_EXISTS` / `INVITATION_EXISTS`; Resend 4xx maps to `INVITATION_EMAIL_REJECTED`.
- **Membership lifecycle.** Creation happens only through `create_member_invitation` (optional first term starting today) and `renew_membership` (next term starts the day after the latest current/queued term; no manual dates). Every genuine renewal stays a separate immutable term/receipt for audit, while `member_access_through` returns the final covered date across all queued terms; member days-left/card/QR use that date immediately. Stable request IDs make retries replay one operation rather than create another term. There is no delete-membership action: a term can only be cancelled through guarded state RPCs. `remove_member` soft-deletes and cancels every live term; `restore_member` leaves terms cancelled. Admin detail exposes current term, next term and payment history.
- **Plans.** `save_membership_plan` creates/edits (name, months, price, description, active). Editing never touches existing memberships (snapshots). `delete_membership_plan` deletes only a plan with zero memberships/UPI requests (`admin_plans()` returns the counts; the UI hides Delete otherwise and says to turn the plan off). Deactivated plans keep members' access to term end and disappear from new-member/renew lists.
- Every mutation is an RPC: `create_member_invitation`, `renew_membership` (**no manual start dates — pass `p_start_date: null`**), `set_membership_state`, `check_in_member`, `check_in_by_qr`, `publish_notice`, `mark_notice_read`, `update_member_profile`, `update_admin_profile`, `update_club_contact`, `update_club_hours` (seven structured day rows shown on member Home/welcome), `issue_qr_pass(p_revoke_existing)`. Member UPI remains manual verification: create server request → amount-bound `upi://pay` QR/native app handoff → member enters UTR → admin confirms.
- Service-role-only RPCs: `finalize_member_invitation`, `mark_member_invitation_failed`, `member_invitation_auth_user`, `active_admin_profiles`, `bootstrap_admin_profile`, `claim_member_invitation_resend`, `prepare_removed_member_reenrolment`, `apply_push_receipt_updates`. Invitation resend has a 60s cooldown, 10-attempt cap and 24h expiry renewal.
- New identifiers: member number `APX-XXXXXX`, receipt `APX-R-YYYY-XXXXXX`; legacy `MRD-*` values remain readable. QR passes are opaque 64-hex/256-bit tokens; only SHA-256 is stored; single-use; 60s TTL; auto-refresh every 45s keeping one predecessor valid; manual regenerate revokes every prior pass.
...
- Android remote push is intentionally NOT configured (owner decision: no Firebase/FCM). Notices rely on in-app delivery with 30s auto-refresh; local (in-app) notifications work; members are prompted for permission once after sign-in. The Expo project `@scnz/apex` (ID `6b5d5a06-d024-4e46-9aa7-c9371a8ff9ab`) is linked for future use. Do not add Firebase/google-services.json unless the owner asks.
