...
- **Member identity rules (write-once).** `members.email` and `members.national_id` (Aadhaar) are unique and, once the member has activated, immutable. Admins may correct the email only *before* onboarding via `update_member_email` (+ Edge `update-member-email`, which also moves the linked auth account and re-targets the pending invitation). Aadhaar can never change after being set (`members_national_id_write_once` trigger). Duplicate Aadhaar/email raise distinct `23505` messages (`members_national_id_unique_message` trigger) so the Edge Function can map them to `AADHAAR_EXISTS` vs `INVITATION_EXISTS`; a Resend 4xx rejection maps to `INVITATION_EMAIL_REJECTED` and the stored `member_invitations.last_error` is shown to admins in `member_detail`.
- **Membership lifecycle.** Creation happens only through `create_member_invitation` (optional first term starting today) and `renew_membership` (next term always starts the day after the current/queued term ends — no manual start dates). There is no "delete membership": a single term can only be cancelled via `set_membership_state('cancelled')` (not exposed in the UI); `remove_member` soft-deletes the member, cancels every live term, and blocks all further writes via `guard_removed_member_write`; `restore_member` reactivates the account but leaves terms cancelled (renew to grant access). Admin `member_detail` exposes every term and payment (`upcomingMembership`, `payments[]` on the client) so queued renewals and their receipts are always visible.
